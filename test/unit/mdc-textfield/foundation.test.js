...
const setupTest = () => {
  ...
  const foundation = new MDCTextFieldFoundation(input, bottomLine, helperText, mockAdapter);
  return {foundation, input, bottomLine, helperText, mockAdapter};
};
...
const setupBlurTest = () => {
  const {foundation, mockAdapter, helperText, input} = setupTest();
  let blur;
  td.when(mockAdapter.registerInputEventHandler(
    MDCTextFieldInputFoundation.strings.BLUR_EVENT, td.matchers.isA(Function)))
    .thenDo((evtType, handler) => {
  ...
  return {foundation, mockAdapter, helperText, input, blur};
};
...
test('on MDCTextFieldInput:blur event, set validity of helper text', () => {
  const {blur, input, helperText} = setupBlurTest();
  input.checkValidity = () => false;
  blur();
